...
class PromptServer():
    def __init__(self, loop):
        ...
        async def get_user_css(request):
            user_css_path = self.user_manager.get_request_user_filepath(request, "user.css", create_dir=False)
            if user_css_path and os.path.isfile(user_css_path):
                return web.FileResponse(user_css_path, headers={"Content-Type": "text/css"})
            return web.Response(text="/* no custom styles */", content_type="text/css")
        ...
        async def view_metadata(request):
            # Metadata viewing not supported in ComfyRaw
            return web.Response(status=404)

        @routes.get("/system_stats")
        async def system_stats(request):
            ram_total = comfy.model_management.get_total_memory()
            ram_free = comfy.model_management.get_free_memory()

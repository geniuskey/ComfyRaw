...
from ._input import ImageInput, AudioInput, MaskInput, LatentInput
from ._util import MESH, VOXEL, File3D
    ...
    class Execution(ProxiedSingleton):
        async def set_progress(
            ...
            if to_display is not None:
                ...
                if isinstance(to_display, np.ndarray):
                    # Convert numpy array to PIL Image
                    # Handle array shape [B, H, W, C] -> get first image if batch
                    arr = to_display
                    if len(arr.shape) == 4:
                        arr = arr[0]

                    # Convert to uint8 and scale to 0-255
                    image_np = (arr * 255).astype(np.uint8)

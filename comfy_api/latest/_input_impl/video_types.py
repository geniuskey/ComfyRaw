        ...
        try:
            ...
            for stream in container.streams:
                ...
                if len(audio_frames) > 0:
                    ...
                    audio_array = np.expand_dims(audio_data, 0)  # shape: (1, channels, total_samples)
                    audio = AudioInput({
...
class VideoFromComponents(VideoInput):
    ...
    def save_to(
        ...
        with av.open(path, mode='w', options={'movflags': 'use_metadata_tags'}, **extra_kwargs) as output:
            ...
            # Encode video
            for i, img_frame in enumerate(self.__components.images):
                img = np.clip(img_frame * 255, 0, 255).astype(np.uint8)  # shape: (H, W, 3)
                video_frame = av.VideoFrame.from_ndarray(img, format='rgb24')
                video_frame = video_frame.reformat(format='yuv420p')  # Convert to YUV420P as required by h264
            ...
            if audio_stream and self.__components.audio:
                ...
                # Convert: movedim(2, 1) → moveaxis(2, 1), reshape, float
                audio_data = np.moveaxis(waveform, 2, 1).reshape(1, -1).astype(np.float32)
                layout = 'mono' if waveform.shape[1] == 1 else 'stereo'
                audio_frame = av.AudioFrame.from_ndarray(audio_data, format='flt', layout=layout)

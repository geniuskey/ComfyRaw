...
class Latent(ComfyTypeIO):
    ...
    class LatentDict(TypedDict):
        samples: np.ndarray
        '''Latent tensors.'''
        noise_mask: NotRequired[np.ndarray]
...
class Conditioning(ComfyTypeIO):
    class PooledDict(TypedDict):
        ...
        cross_attn_controlnet: NotRequired[np.ndarray]
        '''CrossAttn from CLIP to use for controlnet only.'''
        pooled_output_controlnet: NotRequired[np.ndarray]
        '''Pooled output from CLIP to use for controlnet only.'''
        gligen: NotRequired[tuple[str, Gligen, list[tuple[np.ndarray, int, ...]]]]
        ...
        concat_latent_image: NotRequired[np.ndarray]
        '''Used for inpainting and specific models.'''
        concat_mask: NotRequired[np.ndarray]
        '''Used for inpainting and specific models.'''
        concat_image: NotRequired[np.ndarray]
        ...
        attention_mask: NotRequired[np.ndarray]
        ...
        '''Used by unCLIP.'''
        conditioning_lyrics: NotRequired[np.ndarray]
        '''Used by AceT5Model.'''
        ...
        '''Used by CLIPTextEncodeSDXL.'''
        reference_latents: NotRequired[list[np.ndarray]]
        ...
        ref_latent: NotRequired[np.ndarray]
        '''Used by Hunyuan ImageToVideo.'''
        keyframe_idxs: NotRequired[list[int]]
        '''Used by LTXV.'''
        frame_rate: NotRequired[float]
        '''Used by LTXV.'''
        stable_cascade_prior: NotRequired[np.ndarray]
        ...
        vace_frames: NotRequired[np.ndarray]
        '''Used by WAN VACE.'''
        vace_mask: NotRequired[np.ndarray]
        '''Used by WAN VACE.'''
        vace_strength: NotRequired[float]
        '''Used by WAN VACE.'''
        camera_conditions: NotRequired[Any] # TODO: assign proper type once defined
        '''Used by WAN Camera.'''
        time_dim_concat: NotRequired[np.ndarray]
        '''Used by WAN Phantom Subject.'''
        time_dim_replace: NotRequired[np.ndarray]
        '''Used by Kandinsky5 I2V.'''

    CondList = list[tuple[np.ndarray, PooledDict]]
    Type = CondList
